...
supported
* `directtcp-ip` - Forwards a port from the client machine to the remote
  machine.
This is the same as `ssh -nNT -L 8080:forward.example.com:9000
user@sshserver.example.com`
* `direct-streamlocal` - Forwards to a unix domain socket.
This is the same as `ssh -nNT -L 8080:/var/lib/mysql/mysql.sock
user@sshserver.example.com` When using `direct_tcpip/3` or
`direct_stream_local/2` directly there will not be any local port or socket
bound, this can either be done using `ssh_tunnel` or by manually sending data
with `ssh_connection:send/3`. Although `connect/3` can be used to connect to the
remote host, other methods are supported. One can use
[SSHex](https://github.com/rubencaro/sshex), `ssh:connect/3` for instance.

## Tunnels Tunnels are on-demand TCP servers and are bound listeners to either a
port or a path. The tunnel will handle relaying TCP messages to the ssh
connection and back.
...
-doc """
Starts a SSHTunnel.Tunnel process. The tunnel will listen to either a local port
or local path and handle passing messages between the TCP client and ssh
connection. ## Examples {ok, SSH} = ssh_tunnel:connect("sshserver.example.com",
22), {ok, Pid} = ssh_tunnel:start_tunnel(Pid, tcp, {8080, {"192.168.90.15",
80}}) # Send a TCP message %HTTPoison.Response{body: body} =
HTTPoison.get!("127.0.0.1:8080") IO.puts("Received body: #{body})
""".
...
can be used to send messages with `ssh_connection:send/3` returns: `{ok,
channel}` or `{error, reason}`. ## Examples: msg = "GET / HTTP/1.1\r\nHost:
localhost:8080\r\nUser-Agent: curl/7.47.0\r\nAccept: */*\r\n\r\n" {ok, Pid} =
ssh_tunnel:connect("192.168.1.10", 22), {ok, Ch} = ssh_tunnel:direct_tcpip(Pid,
{"127.0.0.1", 8080}, {"192.168.1.10", 80}), ok = ssh_connection:send(Pid, Ch,
Msg), recieve do {ssh_cm, _, {data, Channel, _, Data}} -> io:format("Data:
~p\n", [Data]) end
""".
